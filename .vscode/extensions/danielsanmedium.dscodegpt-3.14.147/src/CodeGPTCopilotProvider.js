    ...
    if (this.lastCompletionStartPosition && this.lastCompletion.length > 0) {
      ...
      if (typedText && insertedText.startsWith(typedText)) {
        ...
        if (remainingText) {
          ...
          const newCompletion = new vscode.InlineCompletionItem(remainingText, range, {
    ...
    return new Promise((resolve) => {
      timerId = setTimeout(async () => {
        ...
        try {
          ...
          if (this.lastProvider !== provider) numWarnings = DEFAULT_NUM_WARNINGS
          ...
          switch (completionProvider) {
            ...
            case 'Mistral':
              ...
              break
            default:
              tooltipMessage = error.message === 'AUTOCOMPLETE_LIMIT_REACHED' ? 'Autocomplete limit reached' : 'Something went wrong'
              break
          }
              ...
              vscode.window
                ...
                .then((selection) => {
                  if (selection === 'Upgrade') {
                    vscode.env.openExternal(
                      vscode.Uri.parse(
                        'https://app.codegpt.co/en/extensions/plans?utm_source=high-demand'
                      )
                    )
                  }
                })
          ...
          lastWarningMessage !== tooltipMessage && numWarnings <= 0 ? numWarnings = DEFAULT_NUM_WARNINGS : numWarnings--
